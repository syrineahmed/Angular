...
import {Residence} from "../models/Résidence";
...
export class CoreComponent {
  ...
  listApartments:Apartment[]=[
    ...
    showAppartments(residence:Residence){
      this.selectedResidenceApartments=this.listApartments.filter((apartment:{residence:{id:number;};}) =>apartment.residence.id === residence.id);
    }
   /* showapp (id:number){
      this.listFilter.filter()
    }*/
    //methode filtrage t afichilk kol residence shnoa aprtmnt mteih
  /*
    showApp(id: number) {
    this.listFiltre = this.listApartments.filter(
      (apart: Apartment) => apart.residence.id == id
   */
    }
